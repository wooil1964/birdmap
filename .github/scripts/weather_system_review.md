...
Actions는 KST 05:35/10:17/14:17/18:17, 작업 제한 30분이다. 첫 실행은 같은 동시실행 그룹의 06:05 조석 갱신과 겹치지 않도록 앞당겼다. 예정 실행 시각에서 30분이 지난 뒤에는 해당 실행 시각보다 이전에 생성된 자료를 참고로 처리한다. 같은 날이라도 갱신 지연·재사용이면 오늘 점수에 사용하지 않는다. GitHub 스케줄 지연 시에도 보수적으로 미확인 표시한다. 자정 이후 전날 자료 역시 참고다.
