...
- 출현종·제보자 이력 관리와 모바일 지원(2026-09-20, 최종 커밋 `3d38e09`): 승인된 제보를 **날짜별 이력**으로 보존하고 지도 팝업에서 조회하게 했다. 로그인 없는 제보, 관리자 승인 뒤 공개, Turnstile, Cloudflare Access 는 그대로다.
  ...
  - **이력 저장 구조**: 한 제보 = 한 관찰일 + 한 제보자이므로, 여러 종을 한 번에 올려도 종↔관찰일↔제보자가 한 행에 정확히 묶인다. 같은 장소에 같은 종이 다른 날짜로 오면 **각각 별도 행으로 남고 덮어쓰지 않는다**. `observed_on`(관찰일)과 `received_at`(접수일)은 분리되어 있고, 관찰일이 없으면 임의로 채우지 않는다. 종별 조회는 구분자를 포함한 정확 일치라 `동박새` 검색이 `한국동박새` 를 끌어오지 않는다.
  - **이름 공개 동의**: 제보 화면의 체크박스는 **기본 해제**이고 값은 `name_public` 에 저장된다. 공개 API 는 `name_public=1` 이고 이름이 실제로 있을 때만 `reporter` 필드를 만든다. 동의하지 않았거나 이름이 없으면 **필드 자체가 없고** 화면에서 `익명 제보` 로 표시된다. 값이 없는 과거 자료는 기본값 0 이라 소급 공개되지 않는다. 관리자 화면의 `이름 공개 반영`(consent) 로 공개를 철회하면 **이름은 비공개로 보존되고 제보는 지워지지 않는다**.
  - **merge → link 교체**: 예전 `merge` 는 대상 지점의 `species` 문자열을 덮어썼다. 이제 `link` 는 `spot_key` 만 지정하고 **원본 종 목록을 고쳐 쓰지 않으며**, 공개 API 가 읽을 때 합집합으로 계산한다. 연결된 제보도 고유 ID 를 유지해 개별 이력 조회와 공개 취소가 그대로 된다. `unlink` 로 연결을 끊을 수 있다. **좌표가 가깝다는 이유로 자동 연결·자동 병합하지 않는다.**
  - **기존 붉은 점 3개 보호**: `index.html` 의 `siteSpotData` 가 정본이고 좌표·출현종은 바꾸지 않는다. 연결용 `id`(`fixed:21:0`, `fixed:195:0`, `fixed:195:1`)만 더했다. 여기에 붙인 제보는 이력으로만 표시되고 수동 점의 종 목록은 건드리지 않는다. 수동 점의 기존 출현종에는 관찰일 자료가 없으므로 **`관찰일 미확인`** 을 붙여 과거 기록과 구분한다.
  - **팝업 이력 표시**: `출현 이력 · 과거 관찰 기록` 제목 아래 **최근 관찰일부터 5건**을 보이고 나머지는 `더 보기 (n건)` 로 편다. 각 줄은 관찰일·출현종·제보자다. 현재 출현 중인 종으로 오인하지 않도록 제목에 과거 기록임을 적었다.
  - **모바일 개선**: 출현 지점 팝업이 화면 위로 잘리던 것(375×812 에서 top −155px)을 고쳤고, 긴 이력은 팝업 안에서만 스크롤된다. 제보 화면은 동의 체크박스 터치 영역 47px, `제보 신청` 버튼을 모달 바닥에 sticky 로 고정해 스크롤·키보드와 무관하게 닿는다. 지도 터치로 위치를 지정하는 동작도 확인했다.
  - **작업 중 실제로 고친 결함 3건**(모두 라이브 실측으로 원인을 특정했다): ① `더 보기` 버튼을 클릭 도중 DOM 에서 제거해 이벤트가 지도로 새고 팝업이 닫혔다 → 제거 대신 숨기고 전파를 막았다. `popup.update()` 도 뺐다(저장된 content 로 innerHTML 을 되돌려 방금 편 항목을 다시 감춘다). ② 출현 지점 팝업이 `popupopen` 핸들러에서 탐조지 팝업보다 먼저 return 해 모바일 auto-pan 여백이 반영되지 않았다. ③ 동의 항목이 늘어 제보 버튼이 접힌 영역 아래로 내려갔다.
  - **D1 스키마 변경**(운영 데이터 0건 상태에서 무손실): `name_public INTEGER NOT NULL DEFAULT 0` 과 `spot_key TEXT` + 인덱스를 `ALTER TABLE ADD COLUMN` 으로 더하고, 쓰지 않게 된 `merged_into` 를 뺐다. `spot_key` 는 `NULL`=자기 점, `fixed:<siteId>:<n>`=수동 붉은 점, 그 외=다른 제보의 점을 뜻한다. 변경 전 스키마는 `wrangler d1 export` 로 받아 두었고, 복구는 `npx wrangler d1 execute birdmap-reports --remote -c wrangler.public.toml --file=<백업.sql>` 이다. 저장소의 정본은 `reports-api/schema.sql` 이며 새 DB 는 이 파일로 만든다.
  - **자동 테스트 40/40 통과**(기존 32 → 40). 테스트 대역을 손으로 만든 질의 분기에서 **Node 내장 SQLite + 실제 `schema.sql`** 로 바꿔 UNIQUE 인덱스와 `LIKE` 조건까지 실제 SQL 로 검증된다. `weather-proxy` 34/34, 기존 지도 스위트 160 통과·10 실패·7 스킵(실패 10건은 Chromium·Python 미설치 환경 실패로 변경 전과 동일), inline `<script>` 파싱 실패 0.
  - **라이브 실측 결과**: 시험 데이터 9건으로 확인했다. 동의한 제보자 이름 표시, 미동의 `익명 제보`, 같은 종 다른 날짜 2건 보존, 여러 종 제보의 종별 연결, 연결 후에도 기존 종 유지, 특정 이력만 공개 취소, 승인 대기 건과 `note`·`admin_note`·`ip_hash`·`bird_count`·미동의 이름이 공개 API 에 **없음**, 팝업 7건 표시와 `더 보기` 동작, 모바일 잘림 0. 기존 붉은 점 3개의 좌표·출현종, 탐조지 190곳, 기상·조석·한 달 조석·자동 추천·공지도 모두 그대로였다.
  - **시험 데이터 정리 완료**: 9건 모두 `admin_note='TESTDATA-이력검증'` 으로 표시해 넣고, 삭제 전 전체 9·대상 9·보존 0 을 확인한 뒤 일괄 삭제했다(`changes: 9`). **현재 `reports` 0건**이고 테이블과 인덱스 4개는 그대로이며 공개 API 는 `{"spots":[],"fixedSpots":{}}` 다.
...
- 신규 탐조지 추가가 기상·조석 Actions를 멈춘 장애와 복구, 헤더 권역 수 자동화(2026-09-16, 기준 main `6983897`): 월포리해변(ID 193) 추가(`47f3c04`)로 탐조지가 187 → 188이 되자 **기상·조석 두 Actions가 모두 실패**했다. 원인은 테스트의 개체 수 전제와 조석 매핑 필수 조건이다. 복구 커밋은 `6983897`이며 두 워크플로 모두 성공했다.
  ...
  - **신규 탐조지 등록 체크리스트**(다음 작업자는 반드시 함께 확인할 것): ① `index.html`의 `siteData` ② `weather-proxy/src/sites.js`(개수·좌표·env·pelagic이 어긋나면 `update_weather.py`가 중단) ③ `.github/scripts/test_weather.py`의 `runtimeSiteCount`·`commonIdCount`·`siteCount`와 전체 장애 테스트의 `unavailableSiteCount`(총계 − 재사용 1) ④ 조석을 쓰면 `update_tide.py`의 `ADDITIONAL_TIDE_SITES` 목록과 **`tide_station_mapping.json` 항목(필수)** ⑤ `.github/scripts/test_tide.py`의 `siteCount` ⑥ `test_weekly_recommendation.mjs`의 실데이터 개체 수 가드 4곳. `test_weather.py`의 `worker["188"]`과 `test_tide.py` 44~48행의 99는 **탐조지 ID·임의 인자이지 개체 수가 아니므로 바꾸지 않는다**.
  - 복구 후 실제 결과: `weather_today.json` `siteCount:188`·`status:ok`, ID 193 `scoreEligible:true`·92점·★★★★★·예보 2026-09-16 19:15 KST, `weather_week.json` 7일 56 sample, `tide_today.json` `siteCount:100`·ID 193 장항 `DT_0024` 라이브 수집. 성공 런은 [기상 35083825113](https://github.com/wooil1964/birdmap/actions/runs/35083825113), [조석 35083825037](https://github.com/wooil1964/birdmap/actions/runs/35083825037)이다. 공개 지도에서 '오늘 적합도 미확인'이 사라진 것도 확인했다.
  - **ID 193의 조석 매핑은 `needsReview: true`이고 `apiVerifiedAt`은 빈 문자열, `apiVerifiedDates`는 빈 배열이다.** 관측소 코드 `DT_0024`(장항)는 송림갯벌(122)이 쓰는 검증된 코드지만 탐조지-관측소 연결 자체는 아직 라이브 검증 전이다. 실제 검증 없이 이 값들을 채우거나 `needsReview`를 내리지 말 것.
  - 헤더 권역 수 자동화: `index.html` 헤더의 고정 문구 `187개 권역`을 `<span id="mapTitleSiteCount">`로 감싸고, `resultCount`와 같은 `siteData.length`를 넣는 `updateMapTitleSiteCount()`를 `load` 핸들러 첫 줄에서 호출한다. 앞으로 탐조지를 추가하면 헤더가 자동으로 따라간다. 최종 업데이트 날짜·버전 문구·CSS·다른 표시는 건드리지 않았다.
  - 로컬 테스트 환경 메모: 그동안 계속 실패로 남던 주간 추천의 C01 선상 테스트 3건은 코드 문제가 아니라 ① 실제 Python 미설치 ② Windows 콘솔 cp949 인코딩(`북풍`이 깨져 문자열 비교 실패) 탓이었다. Python을 두고 `PYTHONIOENCODING=utf-8`로 실행하면 **84/84 통과**한다. CI(Ubuntu, UTF-8)에서는 원래 정상이었다.
...
- 해리천습지 큰노랑발도요 재관찰 공지(2026-09-11, 시작 main `08c630b`): `notices.json` 맨 앞에 사용자 확인 사실만 담은 공지를 추가하고 `siteIds:[126]`으로 연결했다. 게시 기간은 기존 KST `start`/`end` 방식으로 9/11~9/13(이번 주말까지)이며 관찰 날짜·체류 기간을 뜻하지 않는다. 기존 공지 3건은 내용·연결 모두 동일하다.
  - 공지 추가만으로는 가을 갯벌 3자리를 매향리·유부도·청림운동장이 차지해 ID126이 추천 10곳에서 빠졌다. `renderTodayPanel()`에만 기존 `todayIssueCard()`를 재사용하는 '추천 10곳 외 탐조 이슈 연계 장소' 표시를 추가했다. 기존 추천 후보 적격성·안전 판정을 그대로 통과한 장소만 표시하고, 추천 10곳 및 다른 이슈 카드와 ID 중복을 제거한다. 추천 엔진·점수·계절 비율·조석 기준은 변경하지 않았다. 기존 '이번 주 탐조 포인트'에 ID126을 중복 추가하지 않았다.
  - 검증: 주간 추천 83·공지 KST 9·공지 UI 6(신규 1 포함), 총 98/98 통과. JSON·inline JS 3개·diff check 정상. 현재 실자료 PC 1366×768/모바일 390×844에서 공지 4건과 해리천습지 이슈 카드, 추천 10곳 유지, 가로 넘침 0 확인. 지도 CDN이 로드되지 않는 테스트 환경이라 지도 객체·마커만 대역으로 두고 실제 버튼 → 기존 이동 함수 → ID126 좌표 전달을 확인했다(실제 지도 타일/팝업 시각 검증은 아님).
  - siteData 187개 전체·좌표 완전 동일. 기존 공지 3건, 기상/조석 생성 JSON, station mapping, Worker, 추천 정책 무변경. 변경 파일은 notices.json·index.html·기존 공지 UI 테스트·이 인수인계 문서뿐이다.
...
- 자료 없는 날짜의 '이전 자료' 배지 제거(L05, 2026-09-10, 시작·기준 main `ee9d81a`): **root cause** — `renderMonthTideHtml()`이 `showStaleWarning=day.stale===true&&day.fallbackSource!=="monthly_cache"`로 **조석 값의 유무와 무관하게** 재사용 경고를 결정했다. 그래서 그 날짜에 쓸 수 있는 조석 값이 하나도 없어도 `stale=true`이면 '자료 없음' 문구와 '이전 자료' 배지·주석이 **동시에** 렌더됐다. '자료 없음'과 '과거 자료'는 의미가 다르다.
  ...
  - 세 상태 구분: ① 값이 있는 최신 자료 → 배지 없음(기존과 동일) ② **값이 있는 stale/exact-date 자료 → 기존 '이전 자료' 경고 그대로 유지** ③ 쓸 수 있는 값이 없는 날짜 → '자료 없음'만 표시하고 배지 없음. 한 달 안에 ②와 ③이 섞여도 날짜별로 제 상태가 나온다.
  - **M06 무영향**: `monthTideDayForSite()`는 이미 `v24TideDayEvents(...).length`가 있는 row만 오늘 자료로 채택하므로 no-data row가 fallback으로 올라올 수 없고, fallback 우선순위·`monthFallbackStale` 경고 의미도 그대로다. `todayTideLineText()`는 원래 `if(!events.length)return ''`로 보호되어 있어 수정이 필요 없었고, `monthTideFallbackNoticeHtml()`도 M06 경로상 값이 있는 경우에만 도달한다. **L04 무영향**: 모달 제목의 현재 명칭 우선 표시는 그대로이며 ID 14 제목 `한 달 조석 — 걸매리`, ID 188 월간 버튼 노출도 유지된다.
  - 실데이터 영향 **0건**: 현재 `tide_month.json`의 868개 day row 중 no-data 0·`stale=true` 0으로 이 조합이 존재하지 않는다. 즉 조석 API 부분 실패 시에 대비한 표시 보완이며 합성 fixture로 재현했다.
  - 검증: 기존 `test_month_tide_button.mjs`에 L05 회귀 4개를 추가해 9 → **13**이 됐다(새 파일·새 의존성 없음, L01의 Chromium/CDP 방식 재사용). **수정 전 그중 2개가 실패**하고 수정 후 13/13 통과한다. 전체 회귀: 월간 조석 UI 13·기상 48·주간 추천 83·L01 5·M07 9·M06 19·조석 생성기 20·Worker 33으로 **총 230** 통과, validator 2종 통과, inline JS 3개 문법 정상, runtime/Worker 187/187·불일치 0·siteData 187개 완전 동일·좌표 변경 0. C01 3·H01 6·M01 1·M02 1·물때 6도 통과한다.
  - 변경 파일은 `index.html`·`.github/scripts/test_month_tide_button.mjs`·`HANDOVER.md`뿐이다. 조위값·`stationCode`·`stationName`·`tide_month.json`·`tide_today.json`·`tide_station_mapping.json`·`tide_health.json`·조석 생성기(`update_tide.py`·`update_tide_month.py`)·`update_weather.py`·좌표·siteData·물때 임계값·추천/계절/pelagic·`weather_rules.json`·`notices.json`은 **무변경**이다. L06과 `v24BriefingInterpretation()` timezone 건은 다음 단계에서 별도로 다룬다.

- 월간 조석 모달 제목에 현재 탐조지명 사용(L04, 2026-09-10, 시작·기준 main `6025f5b`): **root cause** — 감사가 지목한 A/B/C 중 **C 단독**이다. `openMonthTideModal()`은 모달을 열 때 `title.textContent='한 달 조석 — '+site.name`으로 **현재 runtime 명칭**을 먼저 넣지만, 월간 문서가 로드된 뒤 `title.textContent='한 달 조석 — '+v23Value(siteMonth.name,site.name)`가 **생성 당시 명칭을 우선해 덮어썼다**. ID 14는 runtime `걸매리`인데 `tide_month.json`의 `name`이 `아산만 삽교호`라 최종 제목이 과거 명칭으로 바뀌었다.
  - 수정 전 재현(실제 Chromium/CDP로 `index.html`을 띄워 `openMonthTideModal('14')` 후 `#monthTideTitle` DOM 확인): `한 달 조석 — 아산만 삽교호`. 데이터 확인: runtime siteData `걸매리`, worker `sites.js` `걸매리`, `tide_station_mapping.json` `걸매리`(tideStationCode `DT_0002`·평택), `tide_month.json`/`tide_today.json`만 `아산만 삽교호`이며 좌표는 세 곳 모두 동일하다. 즉 같은 ID 14를 가리키며 조위·관측소·좌표에는 문제가 없다.
  - 수정은 `index.html`의 표현식 **1개**뿐이다: `v23Value(siteMonth.name,site.name)` → `v23Value(site.name,siteMonth.name)`(주석 1줄 포함 실질 3줄). `v23Value(v,fallback)`는 v가 비어 있을 때만 fallback을 쓰므로 **현재 runtime 명칭 우선 · runtime 명칭이 없을 때만 월간 명칭 fallback**이라는 일반 우선순위가 된다. **ID 14 하드코딩은 없다** — 향후 다른 탐조지 이름이 바뀌어도 자동으로 현재 명칭이 표시된다.
  - 수정 후 실제 DOM 제목: `한 달 조석 — 걸매리`. 모달의 `aria-labelledby`가 같은 `#monthTideTitle`을 가리키므로 접근성 label도 함께 교정된다. `siteMonth.name`이 사용자에게 노출되는 곳은 이 제목 하나뿐이며(`renderMonthTideHtml`은 날짜·조위만 사용, `stationName`은 화면에 표시되지 않음) 전역 명칭 체계 리팩터링은 하지 않았다.
  - 다른 월간 site 조사: 월간 대상 **28곳 중 이름 일치 27곳 · 불일치 1곳(ID 14뿐)**. 일치하는 27곳은 두 이름이 같아 표시 결과가 바뀌지 않는다. `tide_today.json`도 같은 ID 14 한 곳만 불일치다. **불일치 데이터를 이번에 고치지 않았다** — 월간 JSON의 `name`은 생성 당시 source metadata이고 조위 자체는 정상이므로 표시 단계에서만 해결했다.
  - 조석 무결성: ID 14의 `stationCode` `DT_0002`·`stationName` 평택·날짜 31일(2026-09-10~10-10)·첫날 만조 `04:02, 16:18 / 896.0, 850.0`·마지막날 만조 `04:13, 16:34 / 889.0, 907.0`·days 해시 모두 수정 전후 동일하다. `tide_month.json`·`tide_today.json`·`tide_station_mapping.json`·`tide_health.json`·`update_tide.py`·`update_tide_month.py`·Excel MasterDB는 **byte 무변경**이고 `TODAY_MUDFLAT_TIDE_RULES`(유부도 700·매향리 850·걸매리 850)도 그대로다. 표시명 수정과 조위 임계값을 섞지 않았다.
  - 검증: 기존 `test_month_tide_button.mjs`에 L04 회귀 4개를 추가해 5 → **9**가 됐다(새 테스트 파일 없음). L01에서 쓰던 Chromium/CDP 방식을 그대로 재사용해 새 의존성 설치 없이 실제 DOM 제목을 읽으며, ID 14 데이터 대조·ID 14 제목이 현재 명칭·이름이 같은 일반 site(ID 19) 제목 불변·runtime 명칭이 빈 경우 월간 명칭 fallback을 검증한다. **수정 전 ID 14 제목 테스트가 `expected '한 달 조석 — 걸매리' / actual '한 달 조석 — 아산만 삽교호'`로 실패**하고 수정 후 9/9 통과한다. 전체 회귀: 월간 버튼 9·기상 48·주간 추천 83·L01 5·M07 9·M06 19·조석 생성기 20·Worker 33으로 **총 226** 통과, validator 2종 통과, inline JS 3개 문법 정상, runtime/Worker 187/187·불일치 0·siteData 187개 완전 동일·좌표 변경 0.
  - 변경 파일은 `index.html`·`.github/scripts/test_month_tide_button.mjs`·`HANDOVER.md`뿐이다. `update_tide.py`·조석 생성 데이터·station mapping·좌표·siteData·`weather_rules.json`·`notices.json`·validator 2종·추천/계절/pelagic 로직은 무변경이다. L05(no-data 이전자료 badge)·L06(문서 정리)과 `v24BriefingInterpretation()` timezone 건은 이번 범위가 아니며 손대지 않았다.
...
- **출현종 제보 시스템 운영·유지보수 메모**(기능 자체는 가동 중이며 보류 사항이 아니다).
  ...
  - **공개 Worker**(`birdmap-reports`, Access 없음): `POST /reports` 로 접수하고 `GET /reports/approved` 로 승인된 것만 내보낸다. 응답은 `spots[]`(각 점의 `id`·좌표·종 합집합·`history[]`)와 `fixedSpots`(수동 붉은 점에 붙은 이력)다. 이력 한 줄은 `{id, date, species, reporter?}` 뿐이고 `reporter` 는 공개 동의한 경우에만 들어간다.
  - **관리자 Worker**(`birdmap-reports-admin`, Access 뒤): 액션은 `approve`·`reject`·`unpublish`·`link`·`unlink`·`consent` 여섯 가지다. 목록 조회는 `?status=`, 종별 이력은 `?species=`, 지점별 이력은 `?spotKey=` 로 부른다. 승인할 때 종명·관찰일·좌표·공개 좌표·이름 공개 동의를 함께 고칠 수 있고, 보내지 않은 값은 원래 값을 유지한다.
  - **한계**: 완전한 스팸 차단은 구조적으로 불가능하다. Turnstile 은 사람이 손으로 넣는 허위 제보를 막지 못하고 IP 기준 제한은 공유 IP·VPN 앞에서 약해진다. 실질적 방어선은 '관리자 승인 전 비공개'다.
- **[확인 필요] 모바일 관리자 화면과 소프트 키보드 동작은 아직 실측하지 않았다.** 검증 완료로 읽지 말 것.
  - 관리자 화면은 Cloudflare Access 로그인이 필요해 자동화 브라우저로 들어갈 수 없었다. 560px 이하 전용 레이아웃(세로 배치, 탭 가로 스크롤, 입력 16px, 버튼 44px)을 넣었을 뿐 **휴대폰에서 승인·공개 취소가 실제로 되는지는 확인하지 못했다.**
  - 실제 스마트폰의 소프트 키보드가 올라온 상태도 에뮬레이터로는 재현할 수 없었다. 제보 버튼을 모달 바닥에 sticky 로 고정해 대비했을 뿐이다.
  - 휴대폰으로 `/admin` 에 접속해 승인·공개 취소를, 제보 화면에서 키보드가 올라온 상태를 확인한 뒤 이 항목을 갱신할 것.
...
- **현재 미반영은 5곳(ID 161·166·171·177·184)뿐이다.** 과거에 '보류 24개(ID 161, 164~186)'로 적혀 있던 후보 중
  **19곳(ID 164·165·167·168·169·170·172·173·174·175·176·178·179·180·181·182·183·185·186)은 이미 runtime `siteData`와
  Worker `sites.js` 양쪽에 반영 완료**다(2026-09-10 main 실측, runtime/Worker 187/187).
  - **[과거 기록]** 당시 문장: "신규 탐조지 후보 중 24개(ID 161, 164~186): 엑셀 정제본에는 포함되었으나 지도 반영은 보류 상태."
    이 24개 목록은 그 시점 기록이며 현재 보류 목록이 아니다.
  - 남은 5곳을 반영할 때는 `AI_WORK_RULES.md` 5항(좌표 검증)과 7항(기상 이중 관리)을 반드시 준수할 것.
